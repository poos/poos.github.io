...
的膜比较简单，如果碰到问题可以回顾一下这个博客，里边有详细的步骤。[Sourcery Tutorial: Generating Swift code for iOS](https://www.raywenderlich.com/501-sourcery-tutorial-generating-swift-code-for-ios)
...
而且可以看到，除了普通参数生成，作者还特意标注了 isArray， isDictionary 的取值方法。但是还是会有问题，如果多个数组需要不同的取值，用这种方式就不能行得通了。
...
可以看到，我在 firstName， lastName 等参数上边写了注释，这也同样是文章开头那篇博客里提到的，但是博客里面缺少太多的示例代码。

我通过 github 搜索，在 [aichiko0225/LJInstalment](https://github.com/aichiko0225/LJInstalment) 这个工程里面找到了示例
